...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.error) {
        setError(response.error.message || '注册失败，请重试');
      } else {
        setSuccess(true);
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-slate-900 to-neutral-900 flex items-center justify-center p-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-0 right-1/3 w-96 h-96 bg-slate-600 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse animation-delay-2000"></div>
      </div>
      ...
      <div className="relative w-full max-w-md">
        <div className="backdrop-blur-xl bg-white/10 rounded-2xl shadow-2xl border border-white/20 p-8 space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-gray-300">加入我们的专业社区</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="输入您的昵称"
                className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition backdrop-blur-sm"
              />
            </div>
